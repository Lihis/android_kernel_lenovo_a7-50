...
const USHORT otp_reglist[LSC_REGSIZE] = 
{
	0x381e, //LSC Start
	0x3820,
	0x3822,
...
const USHORT lsc_reglist[LSC_REGSIZE] = 
{
   //A5141 A8141
	0x3600, 
	0x3602, 
...
bool start_read_otp(USHORT zone)
{
	...
	//SENSORDB("[A5142_OTP]record_type = 0x%04x\n",record_type);
...
//	A5142MIPI_write_cmos_sensor(0x304A, 0x0200); // auto read start  0x0010
	...
	if (cnt == 50)
	{
		SENSORDB("[A5142_OTP]POLL R0x304A[6:5] = 11 failed\n");	

		SENSORDB("[A5142_OTP]otp read type:0x%02x,temp:%x error\n", zone|0x30,temp);

		return KAL_FALSE;
	}

	// check data validation
	temp = A5142MIPI_read_cmos_sensor(0x38F2);
	if (temp != 0xFFFF) 
	{
		SENSORDB("[A5142_OTP]record type 0x%x is invalid because of the 0x38F2\n", record_type);
		return KAL_FALSE;
	}
...
USHORT get_otp_module_id(USHORT zone)
{
	...
	if(!start_read_otp(zone))
	{
		SENSORDB("[A5142_OTP]DBG_DH:Otp read start Fail!\n");
		return 0;
	}

	module_id = A5142MIPI_read_cmos_sensor(0x3806);

	SENSORDB("[A5142_OTP]DBG_DH:Module ID: 0x%04x\n", module_id);
...
USHORT get_otp_lens_id(USHORT zone)
{
	...
	if(!start_read_otp(zone))
	{
		SENSORDB("[A5142_OTP]read start Fail!\n");
		return 0;
	}

	lens_id = A5142MIPI_read_cmos_sensor(0x3808);

	SENSORDB("[A5142_OTP]Lens ID: 0x%04x.\n",lens_id);
...
USHORT get_otp_vcm_id(USHORT zone)
{
	...
	SENSORDB("[A5142_OTP]VCM ID: 0x%04x.\n",vcm_id);
...
USHORT get_otp_driver_id(USHORT zone)
{
	...
	SENSORDB("[A5142_OTP]Driver ID: 0x%04x.\n",driver_id);
...
**************************************************************************************************/
bool otp_lenc_update(USHORT zone, bool data_backup)
{
	USHORT IsShading;
	int i;
	USHORT *otp_srt;
	...
	if(!start_read_otp(zone))
	{
		SENSORDB("[A5142_OTP]DBG_DH:LSC update err\n");
		return KAL_FALSE;
	}
	...
	if(otp_lsc == NULL)
	{
		SENSORDB("[A5142_OTP]Create otp_lsc buff err!\n");
		return KAL_FALSE;
	}
	...
	if (data_backup)
	{
		for(i=0;i<LSC_REGSIZE;i++)
		{
			...
			SENSORDB("[A5142_OTP]REG = 0x%04x , 0x%04x\n",*(otp_srt + i),*(otp_lsc + i));
		}
	}
	else
	{
	...
	SENSORDB("[A5142_OTP]DBG_DH:LSC update from OTP[zone:%d] Okay!\n",zone);

	return KAL_TRUE;
}


void otp_LensData_backup(void)
{
	int i;
	USHORT *otp_srt;
...
int wb_multi_cal(USHORT value)
{
	...
	{
		SENSORDB("[A5142_OTP]Digital_Gain Err\n");
		return 0;
	}
...
bool wb_gain_set(int r_ratio, int b_ratio)
{
	...
	if(!r_ratio || !b_ratio)
	{
		SENSORDB("[A5142_OTP]DBG_DH:OTP WB ratio Data Err\n");
		return KAL_FALSE;
	}
...
{
	USHORT golden_rg_ratio, golden_bg_ratio, current_rg_ratio,current_bg_ratio,current_GbGr_ratio;
	USHORT rg_ratio,bg_ratio;
	
	if(!start_read_otp(zone))
	{
		SENSORDB("[A5142_OTP]DBG_DH:AWB update err\n");
		return KAL_FALSE;
	}
	
	current_rg_ratio  = A5142MIPI_read_cmos_sensor(0x3810);     //MTK request
	current_bg_ratio  = A5142MIPI_read_cmos_sensor(0x3812);     //MTK request
	//current_GbGr_ratio = A5142MIPI_read_cmos_sensor(0x3814);     //MTK request

	// R/G and B/G should be fixed for lenovo
	golden_rg_ratio = 0x2b0;//A5142MIPI_read_cmos_sensor(0x3816);     //MTK request
	golden_bg_ratio  = 0x258;//A5142MIPI_read_cmos_sensor(0x3818); //MTK request


	if(!golden_rg_ratio || !golden_bg_ratio || !current_rg_ratio || !current_bg_ratio/* || !current_GbGr_ratio*/)
	{
		SENSORDB("[A5142_OTP]DBG_DH:wb update err\n");
		return KAL_FALSE;
	}

	rg_ratio = 100 * golden_rg_ratio / current_rg_ratio;
	bg_ratio = 100 * golden_bg_ratio / current_bg_ratio;

	SENSORDB("[A5142_OTP]crt_rg_ratio:%x,crt_bg_ratio:%x,rg_ratio:%d,bg_ratio:%d\n",current_rg_ratio,current_bg_ratio,rg_ratio,bg_ratio);
	
	wb_gain_set(rg_ratio, bg_ratio);

	SENSORDB("[A5142_OTP]DBG_DH:AWB OTP[zone:%d] update Okay\n",zone);	
