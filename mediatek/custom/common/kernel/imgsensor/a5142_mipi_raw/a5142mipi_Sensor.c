...
#define SENSORDB(fmt, arg...) printk("[A5142MIPI_DEBUG]%s: " fmt "\n", __FUNCTION__ ,##arg)
#else
#define SENSORDB(x,...)
#endif
...
#include "a5142_otp.h"
bool data_validate_page30 = KAL_FALSE;
#endif
...
UINT32 A5142MIPISetMaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId,UINT32 frameRate)
{
    ...
    switch(scenarioId) {
        case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
            ...
            dummyLine = frameHeight - A5142MIPI_PV_PERIOD_LINE_NUMS;
			if (dummyLine<=0)
				dummyLine = 0;
        ...
        case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
            ...
            frameHeight = (10*pclk)/frameRate/lineLength;
            dummyLine = frameHeight - A5142MIPI_FULL_PERIOD_LINE_NUMS;
			if (dummyLine<=0)
				dummyLine = 0;
...
#if defined(OTP_SUPPORT)
	if (!data_validate_page30)	// page31
	{
		otp_lenc_update(1, KAL_TRUE);
		otp_wb_update(1);
	}
	else						// page30
	{
		otp_lenc_update(0, KAL_TRUE);
		otp_wb_update(0);
	}
	printk("A5142MIPI_Sensor_Init data_validate_page30:%x", data_validate_page30 );
#endif // OTP_SUPPORT
    //mDELAY(20);  // zjz              // Allow PLL to lock
...
UINT32 A5142MIPIGetSensorID(UINT32 *sensorID) 
{
    ...
    *sensorID  = sensor_id;
    ...
    SENSORDB("A5142MIPIGetSensorID sensor_id is %x ", *sensorID );
...
#if defined(OTP_SUPPORT)
{
	static kal_bool check_first = KAL_FALSE;

	if (check_first)
		goto exit;
	
	if (start_read_otp(1))	// page31
	{
		data_validate_page30 = KAL_FALSE;
	}
	else					// page30
	{
		start_read_otp(0);
		data_validate_page30 = KAL_TRUE;
	}
	otp_LensData_backup();
	check_first = KAL_TRUE;
}
#endif // OTP_SUPPORT
...
UINT32 A5142MIPIOpen(void)
{
    ...
    SENSORDB("A5142MIPIOpen sensor_id is %x ", sensor_id);
	...
	#if 0//def OTP_SUPPORT
	SENSORDB("[0x3804]=%x", A5142MIPI_read_cmos_sensor(0x3804));
	SENSORDB("[0x3816]=%x", A5142MIPI_read_cmos_sensor(0x3816));
	SENSORDB("[0x3818]=%x", A5142MIPI_read_cmos_sensor(0x3818));
	SENSORDB("[0x38F2]=%x", A5142MIPI_read_cmos_sensor(0x38F2));
	#endif
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    pSensorInfo->PreviewDelayFrame = 1; // 2 zjz 
