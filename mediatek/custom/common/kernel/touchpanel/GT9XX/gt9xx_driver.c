...
static int gt91xx_config_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    ptr += sprintf(ptr, "pid:0x%02x%02x\n", temp_data[5], temp_data[4]);
...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    u8 *send_cfg_buf_charger[6] = {cfg_info_group1_charger, cfg_info_group2_charger, cfg_info_group3_charger, cfg_info_group4_charger, cfg_info_group5_charger, cfg_info_group6_charger};
#endif	
    ...
    GTP_DEBUG("len1=%d,len2=%d,len3=%d,len4=%d,len5=%d,len6=%d,get_len=%d", cfg_info_len[0], cfg_info_len[1], cfg_info_len[2], cfg_info_len[3], cfg_info_len[4], cfg_info_len[5], cfg_len);
    ...
    else
    {
        ...
        if (cfg_info_len[rd_cfg_buf[GTP_ADDR_LENGTH]]  == 0)
        {
            goto out;
        }
    }
