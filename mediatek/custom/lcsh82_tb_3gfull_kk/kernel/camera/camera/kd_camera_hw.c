...
u32 pinSet[2][8] = {
	...
	else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_A5142_MIPI_RAW, currSensorName)))
	{
		...
		//SUB DVDD
		if(TRUE != hwPowerOn(CAMERA_SUB_POWER_VCAM_D, VOL_1800,mode_name))
		{
			PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
			//return -EIO;
 			//goto _kdCISModulePowerOn_exit_;
	...
	if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_A5142_MIPI_RAW, currSensorName)))
	{
		...
		//SUB DVDD
		if(TRUE != hwPowerDown(CAMERA_SUB_POWER_VCAM_D, mode_name))
		{
			PK_DBG("[CAMERA SENSOR] Fail to OFF digital power\n");
			//return -EIO;
			goto _kdCISModulePowerOn_exit_;
