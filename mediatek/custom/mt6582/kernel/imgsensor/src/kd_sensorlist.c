...
//shaohui add
#define LCT_MTK_CCM_INFO_SUPPORT

#ifdef LCT_MTK_CCM_INFO_SUPPORT
#define CCM_PROC_FILE "camera_info"
char mtk_ccm_name[128] = {0};
#endif
...
inline static int adopt_CAMERA_HW_CheckIsAlive(void)
{
    ...
    if (g_pSensorFunc) {
		...
		#ifdef LCT_MTK_CCM_INFO_SUPPORT
			if(sensorID!=0xFFFFFFFF)
			{
				if(DUAL_CAMERA_MAIN_SENSOR == g_invokeSocketIdx[i])
				{
					memset(mtk_ccm_name,0,sizeof(mtk_ccm_name));
				}
				sprintf(mtk_ccm_name, "%s CCM:%d,%s ; ",mtk_ccm_name,g_invokeSocketIdx[i],g_invokeSensorNameStr[i]);
			}
		#endif
		        if(ERROR_NONE != err)
		        {
...
#ifdef LCT_MTK_CCM_INFO_SUPPORT
static int  CAMERA_Dumpinfo_To_Proc(char *page, char **start, off_t off,
                                                                                       int count, int *eof, void *data)
{
	int cnt=0;
	if(off != 0)
...
#ifdef LCT_MTK_CCM_INFO_SUPPORT
    struct proc_dir_entry *prEntry_ccm;
#endif
...
#ifdef LCT_MTK_CCM_INFO_SUPPORT
	memset(mtk_ccm_name,0,128);
    prEntry_ccm = create_proc_entry(CCM_PROC_FILE, 0, NULL); 
    if (prEntry_ccm) {
        prEntry_ccm->read_proc = CAMERA_Dumpinfo_To_Proc; 
        prEntry_ccm->write_proc = NULL; 
    }
    else {
        PK_ERR("add /proc/driver/camsensor entry fail \n");  
    }
