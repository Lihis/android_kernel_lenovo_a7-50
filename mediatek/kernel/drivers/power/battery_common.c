...
#if (defined(MTK_TEMPERATURE_RECHARGE_SUPPORT) || defined(BAT_OVER_TEMP_POWER_OFF))
kal_int32 g_battery_temperature=25;
#endif

#if defined(BAT_OVER_SOC_RECHARGE)
kal_uint32 g_batt_soc_status = SOC_NORMAL;
kal_bool g_battery_test_status = KAL_FALSE;
kal_int32 g_battery_percent = 50;
#endif
...
{
	battery_xlog_printk(BAT_LOG_CRTI, "[Battery] show_BatteryTemperature : %d\n", g_battery_temperature);
...
static ssize_t store_BatteryTemperature(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	...
	battery_xlog_printk(BAT_LOG_CRTI, "[Battery] store_BatteryTemperature\n");
...
static void battery_update(struct battery_data *bat_data)
{
	...
	bat_data->charge_full_design = Q_MAX_POS_25_ATL*1000;
	else
	bat_data->charge_full_design = Q_MAX_POS_25_COS*1000;
#endif
...
{ 
	...
	{
		BMT_status.ICharging = 960;
	}
	else
	{
		BMT_status.ICharging = 576;
	}
	#endif	
...
#endif
#endif
#if defined(BAT_OVER_TEMP_POWER_OFF)
	if(BMT_status.temperature == OVER_TEMP_POWER_OFF_TEMPERATURE)
	{
		#if defined(BAT_OVER_SOC_RECHARGE)
		if (g_battery_test_status == KAL_FALSE)
		{
		...
		g_BatteryNotifyCode |= 0x0080;
		#endif
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of range(power off)\n", BMT_status.temperature);	
	}
#endif
