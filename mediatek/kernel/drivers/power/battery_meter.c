...
void fgauge_get_profile_id(void)
{
	...
	ret = IMM_GetOneChannelValue_Cali(AUXADC_BATTERY_ID_CHANNEL, &id_volt);
	...
	if ((id_volt>0) && (id_volt<1000000))
	{
		g_fg_battery_id = 0;
	}
	else if (id_volt>1000000)
	{
		g_fg_battery_id = 1;
	}
	else
...
kal_int32 fgauge_get_Q_max(kal_int16 temperature)
{
    ...
    if (temperature <= TEMPERATURE_T1)
    {
        ...
        {
        	low_Q_max = Q_MAX_NEG_10_ATL;
        }
        else
        {
        	low_Q_max = Q_MAX_NEG_10_COS;
        }
        high_temperature = TEMPERATURE_T1;
        if (g_fg_battery_id == 0)
        {
        	high_Q_max = Q_MAX_POS_0_ATL;
        }
        else
        {
        	high_Q_max = Q_MAX_POS_0_COS;
        }
    ...
    else if (temperature <= TEMPERATURE_T2)
    {
        ...
        if (g_fg_battery_id == 0)
        {
        	low_Q_max = Q_MAX_POS_0_ATL;
        }
        else
        {
        	low_Q_max = Q_MAX_POS_0_COS;
        }
        high_temperature = TEMPERATURE_T2;
        if (g_fg_battery_id == 0)
        {
        	high_Q_max = Q_MAX_POS_25_ATL;
        }
        else
        {
        	high_Q_max = Q_MAX_POS_25_COS;
        }
    ...
    else
    {
        ...
        if (g_fg_battery_id == 0)
        {
        	low_Q_max = Q_MAX_POS_25_ATL;
        }
        else
        {
        	low_Q_max = Q_MAX_POS_25_COS;
        }
        high_temperature = TEMPERATURE_T3;
        if (g_fg_battery_id == 0)
        {
        	high_Q_max = Q_MAX_POS_50_ATL;
        }
        else
        {
        	high_Q_max = Q_MAX_POS_50_COS;
        }
...
kal_int32 fgauge_get_Q_max_high_current(kal_int16 temperature)
{
    ...
    if (temperature <= TEMPERATURE_T1)
    {
        ...
        if (g_fg_battery_id == 0)
        {
        	low_Q_max = Q_MAX_NEG_10_H_CURRENT_ATL;
        }
        else
        {
        	low_Q_max = Q_MAX_NEG_10_H_CURRENT_COS;
        }
        high_temperature = TEMPERATURE_T1;
        if (g_fg_battery_id == 0)
        {
        	high_Q_max = Q_MAX_POS_0_H_CURRENT_ATL;
        }
        else
        {
        	high_Q_max = Q_MAX_POS_0_H_CURRENT_COS;
        }
    ...
    else if (temperature <= TEMPERATURE_T2)
    {
        ...
        if (g_fg_battery_id == 0)
        {
        	low_Q_max = Q_MAX_POS_0_H_CURRENT_ATL;
        }
        else
        {
        	low_Q_max = Q_MAX_POS_0_H_CURRENT_COS;
        }
        high_temperature = TEMPERATURE_T2;
        if (g_fg_battery_id == 0)
        {
        	high_Q_max = Q_MAX_POS_25_H_CURRENT_ATL;
        }
        else
        {
        	high_Q_max = Q_MAX_POS_25_H_CURRENT_COS;
        }
    ...
    else
    {
        ...
        if (g_fg_battery_id == 0)
        {
        	low_Q_max = Q_MAX_POS_25_H_CURRENT_ATL;
        }
        else
        {
        	low_Q_max = Q_MAX_POS_25_H_CURRENT_COS;
        }
        high_temperature = TEMPERATURE_T3;
        if (g_fg_battery_id == 0)
        {
        	high_Q_max = Q_MAX_POS_50_H_CURRENT_ATL;
        }
        else
        {
        	high_Q_max = Q_MAX_POS_50_H_CURRENT_COS;
        }
...
BATTERY_PROFILE_STRUC_P fgauge_get_profile(kal_uint32 temperature)
{
    switch (temperature)
    {
        case TEMPERATURE_T0:
            if (g_fg_battery_id == 0)
            {
            	return &battery_profile_t0_atl[0];
            }
            else
            {
            	return &battery_profile_t0_cos[0];
            }
            break;
        case TEMPERATURE_T1:
            if (g_fg_battery_id == 0)
            {
            	return &battery_profile_t1_atl[0];
            }
            else
            {
            	return &battery_profile_t1_cos[0];
            }
            break;
        case TEMPERATURE_T2:
            if (g_fg_battery_id == 0)
            {
            	return &battery_profile_t2_atl[0];
            }
            else
            {
            	return &battery_profile_t2_cos[0];
            }
            break;
        case TEMPERATURE_T3:
            if (g_fg_battery_id == 0)
            {
            	return &battery_profile_t3_atl[0];
            }
            else
            {
            	return &battery_profile_t3_cos[0];
            }
            break;
        case TEMPERATURE_T:
...
R_PROFILE_STRUC_P fgauge_get_profile_r_table(kal_uint32 temperature)
{
    switch (temperature)
    {
        case TEMPERATURE_T0:
            if (g_fg_battery_id == 0)
            {
            	return &r_profile_t0_atl[0];
            }
            else
            {
            	return &r_profile_t0_cos[0];
            }
            break;
        case TEMPERATURE_T1:
            if (g_fg_battery_id == 0)
            {
            	return &r_profile_t1_atl[0];
            }
            else
            {
            	return &r_profile_t1_cos[0];
            }
            break;
        case TEMPERATURE_T2:
            if (g_fg_battery_id == 0)
            {
            	return &r_profile_t2_atl[0];
            }
            else
            {
            	return &r_profile_t2_cos[0];
            }
            break;
        case TEMPERATURE_T3:
            if (g_fg_battery_id == 0)
            {
            	return &r_profile_t3_atl[0];
            }
            else
            {
            	return &r_profile_t3_cos[0];
            }
            break;
        case TEMPERATURE_T:
...
//-------------------------------------------------------------------------------------------
#if defined(MTK_MULTI_BAT_PROFILE_SUPPORT)
static ssize_t show_FG_g_fg_battery_id(struct device *dev,struct device_attribute *attr, char *buf)
{    
    bm_print(BM_LOG_CRTI, "[FG] g_fg_battery_id : %d\n", g_fg_battery_id);
    return sprintf(buf, "%d\n", g_fg_battery_id);
}
static ssize_t store_FG_g_fg_battery_id(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    return size;
}
static DEVICE_ATTR(FG_g_fg_battery_id, 0664, show_FG_g_fg_battery_id, store_FG_g_fg_battery_id);
#endif
